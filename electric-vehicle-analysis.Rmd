...
2010 and 2022 in China, Europe, India, USA, the World, and the rest of the World. These graphs 
show that as time progresses, both electricity demand and oil displacement increase in all the 
regions, except India. However, there is only data on the oil displacement and electricity 
demand in India for 2019 through 2022, so it is hard to make any conclusions because there 
aren't enough years of data available to be able to effectively see if there have been any 
significant changes. This graph also shows that electricity demand and oil displacement tend 
to increase with each other, which may be because, as the world moves towards electric vehicles,
demand for oil will be replaced by the demand for electricity.
  ```
...
is still a lot of progress to be made, as electric cars are a very small fraction of the total 
new cars sold. Cars currently in use is also a useful metric, as it can approximately summarize 
car sales for up to the past decade. It takes time for the new sales of electric cars to be 
reflected, so the numbers for current percentages of electric vehicles on the road are lower. 
This can however give hope for lower emissions in years to come since we know that the sales of 
electric cars have been rapidly increasing and so it is only a matter of time before that trend
is reflected in current car use. Our first original graph shows the electricity demand and oil 
displacement and led us to the conclusion that they tend to increase with each other, which could 
be because as we are moving towards electric vehicles, the demand for oil will be replaced by the 
demand for electricity. We also looked at the distribution of electric vehicle sales per year 
across all the regions of the dataset.  We looked at oil displacement as well for several regions 
including the world, and saw that the world saw improvement, but some regions had none at all like 
India. This tells us that some regions have made large increases in sales of electric vehicles, 
while others have made almost none at all, so there is a lot of variability in the responses of 
different regions to the climate crisis. We also took a look at future data, which predicts no 
decrease in the amount of non-electric cars sold, but that electric car sales will continue to 
increase. We took a look at projected electric vehicles for several regions, and it is important
to note that the scales of the facet graphs are different so countries like India are still far 
behind China or the USA. We also sought to confirm the correlation between EVs being sold and oil
displacement, and there seems to be a direct correlation suggesting that increasing EV sales can 
reduce oil consumption, something that was reinforced by our linear regression model. Overall, 
the world is taking positive steps towards reducing oil consumption, but there are some countries 
that are lagging behind in switching towards electric vehicles, which could be holding the rest 
of the world back from making even larger impacts on the environment. 
  ```
